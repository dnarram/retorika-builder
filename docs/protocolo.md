...
| Alojamiento de la aplicación | Render (runtime nativo de Node, desde GitHub, Fráncfort) | Despliegue automático al fusionar y la misma región que Supabase. Node 24 es ya la versión por defecto de los servicios Node de Render, así que el Dockerfile no compraba nada más que un paso de build que mantener (nota del 24 de septiembre de 2026, ADR 0012). En la capa gratuita el servicio se duerme tras unos 15 minutos sin uso: el arranque en frío tarda alrededor de un minuto, y una vez despierto la respuesta baja a 129 ms, así que cualquier demostración en directo necesita una llamada de calentamiento antes. Ninguna web de cliente depende de esto: son archivos estáticos en su propio alojamiento (ADR 0012). |
